...
define('TRANSLATION_CACHE_FILE', __DIR__ . '/translation_cache.php');

// Initialize or load the translation array.
if (file_exists(TRANSLATION_CACHE_FILE)) {
    $translation_array = include TRANSLATION_CACHE_FILE;
} else {
    $translation_array = [];
}
...
function save_translation_array() {
    ...
    // Save to the file.
    file_put_contents(TRANSLATION_CACHE_FILE, $php_code);
}
...
function custom_translation_handler( $translated, $original, $domain ) {
    ...
    if (
        ...
        if (!array_key_exists($original, $translation_array)) {
            $translation_array[$original] = $domain;
        }
    }


    // Check the language stored in the session
    ...
    // Replace translation if available for the selected language
    // if ( $lang === 'ru' && isset( $translations['ru'][ $original ] ) ) {
    //     return $translations['ru'][ $original ];
    // }

    return $translated; // Default translation if no match
}

add_filter('gettext', 'custom_translation_handler', 10, 3);
